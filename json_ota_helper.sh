...
if [ -z "$filename" ]; then
    ...
    echo "Please make sure the input json contains a 'filename' property in the format 'horizon_<codename>-ota-<>.json'"
    exit 1
fi

codename=$(echo "$filename" | sed -E 's/^horizon_([^.-]+)-ota-.+$/\1/')
if [ -z "$codename" ]; then
    display_help
    exit 1
fi
...
url="https://sourceforge.net/projects/horizonui/files/Android%2014/${filename}/download/"
